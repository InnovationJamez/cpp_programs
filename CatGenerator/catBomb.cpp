...
void drawCat(std::string* catString){
	...
	// feet
	(*catString) += drawCircle(10, 91, 5, ((rand() % 10) > 5 ? chestColor : bodyColor));
	(*catString) += drawCircle(25, 91, 5, ((rand() % 10) > 5 ? chestColor : bodyColor));
	(*catString) += drawCircle(55, 91, 5, ((rand() % 10) > 5 ? chestColor : bodyColor));
	(*catString) += drawCircle(70, 91, 5, ((rand() % 10) > 5 ? chestColor : bodyColor));
	...
	// tail
	(*catString) += drawCircle(90, 55, 5, ((rand() % 10) > 5 ? chestColor : bodyColor));
...
int main(){
	...
	srand(time(NULL));
	for(int i = 0; i < 500; i++){
		fileName = "cat" + std::to_string(i)  + ".svg";
		filePath = "files/bomb/" + fileName;
